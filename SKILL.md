...
- **High-Signal / Zero-False-Positive Mandate**: To avoid the "review fatigue" and noise of traditional linters, the auditor is forbidden from flagging purely theoretical issues without empirical verification. Always trace variables and function scopes within the full application context before raising a defect.
- **Deep Supabase RLS & Storage Hardening (`security-rls-basics`)**: Enforce Row Level Security (RLS) on every exposed table. **Strictly flag and reject any policy using `USING (true)` or `WITH CHECK (true)`** on tenant/customer data, as this creates a world-readable/writable exploit with public anon keys. Verify that Supabase Storage buckets (`supabase.storage`) enforce authenticated owner policies matching table RLS.
- **Server Action Mutation Cache Invalidation (`revalidatePath`)**: Every Server Action performing database mutations (`insert`, `update`, `delete`, `upsert`) MUST explicitly invoke `revalidatePath()` or `revalidateTag()` to purge the Next.js router cache and prevent stale UI desynchronization.
- **Optimistic UI Rollback Integrity**: Any component implementing optimistic UI updates (e.g. `useOptimistic`, instant list removal) MUST include deterministic rollback state handling in its `catch` block and display an actionable error notification if the server rejects the mutation.
